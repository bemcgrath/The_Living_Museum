 ...
 * src/data/genreProfiles.ts; never surfaced to a viewer). artist_name IS safe to include — it's the
 * public-facing fictional persona (e.g. "Oscar"), same as api/showcase.ts; null for pieces sent
 * before that column existed. Showcase collection pieces (kind 'showcase') are marketing art, never
 * emailed to anyone — they're served separately by api/showcase.ts and excluded here so they don't
 * dominate this "sent to subscribers" gallery.
 */
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  const { data, error } = await supabase
    .from('pieces')
    .select('id, style, subject, image_url, artist_name, created_at')
